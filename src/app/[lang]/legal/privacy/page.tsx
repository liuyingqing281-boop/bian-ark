...
const ZH: { title: string; updated: string; sections: Section[] } = {
  ...
  sections: [
    ...
    { h: "5. 语音功能（语音输入与朗读）", p: "对话场景的语音输入（将您说出的话转为文字）与朗读（将文字转为语音）会采集您的声纹，声纹属生物识别信息。语音数据仅传输至小米 MiMo API 用于当次识别或合成，不用于模型训练，也不用于任何其他目的；识别结果以文字形式保存，原始音频在任务完成后立即丢弃。用于音色复刻的生前语音样本仅用于生成该纪念馆的合成音色，经人工审核通过后启用，样本在审核完成后按约定期限删除；复刻音色会以「AI 合成声音」标识，且不用于冒充或误导他人。" },
    { h: "6. 信息的共享", p: "不出售您的个人信息。仅在以下情形共享：生成任务所需的国内供应商（阿里云、小米 MiMo、数字人服务商等，均签署数据保密与删除协议）、支付渠道（Stripe）、法律法规要求。" },
    { h: "7. 存储与删除", p: "数据存储于境内服务器。您可随时删除纪念馆、媒体与自定义祭品；删除账号后，其创建的私密纪念馆将被删除，已公开的纪念馆在移除个人信息后保留或按您的要求一并删除。" },
    { h: "8. 您的权利", p: "您有权访问、更正、导出、删除个人信息，有权撤回授权（撤回不影响撤回前基于授权已完成的处理）。通过登录页验证码即可管理账号。" },
    { h: "9. 联系我们", p: "对本政策或逝者数据处理有任何疑问，请通过平台内反馈渠道联系我们，我们将在 15 个工作日内答复。" },
  ],
};
...
const EN: { title: string; updated: string; sections: Section[] } = {
  ...
  sections: [
    ...
    { h: "5. Voice input and reading aloud", p: "Voice input (converting your speech to text) and reading aloud (converting text to speech) in conversations collect your voiceprint, which is biometric information. Voice data is transmitted only to the Xiaomi MiMo API for the current recognition or synthesis — never for model training or any other purpose; recognition results are stored as text and the raw audio is discarded immediately after the task. Pre-recorded voice samples used for voice cloning are used solely to generate that memorial's synthesized voice, are enabled only after human review, and are deleted within the agreed retention window after review; cloned voices are labeled 'AI-synthesized voice' and must not be used to impersonate or mislead." },
    { h: "6. Sharing", p: "We do not sell personal information. We share only with: domestic vendors required to fulfill generation tasks (under confidentiality and deletion agreements), payment processors (Stripe), and where required by law." },
    { h: "7. Storage and deletion", p: "Data is stored on servers in mainland China. You may delete memorials, media and custom offerings at any time. When an account is deleted, its private memorials are removed; public memorials are anonymized or removed on request." },
    { h: "8. Your rights", p: "You may access, correct, export and delete your personal information, and withdraw consent (without affecting processing already completed). Manage your account via code-based sign-in." },
    { h: "9. Contact", p: "Questions about this policy or deceased-person data handling: reach us through the in-product feedback channel. We respond within 15 business days." },
  ],
};
