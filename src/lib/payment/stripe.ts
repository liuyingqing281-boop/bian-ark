...
export class StripePaymentProvider implements PaymentProvider {
  ...
  async createPayment(intent: PaymentIntent): Promise<PaymentResult> {
    ...
    if (intent.kind !== "dh_redo" && (!price || price.includes("xxxx"))) throw new Error("price_not_configured");
    ...
    const session = await getStripe().checkout.sessions.create({ mode: intent.kind === "dh_redo" ? "payment" : "subscription", line_items: lineItems as never, metadata: { kind: intent.kind, user_id: intent.userId, memorial_id: intent.memorialId || "", order_id: intent.orderId }, customer_email: undefined, success_url: `${process.env.PUBLIC_APP_URL || "http://localhost:3003"}/zh/me?paid=1`, cancel_url: `${process.env.PUBLIC_APP_URL || "http://localhost:3003"}/zh/membership?canceled=1` });
