...
const vf = await ownerApi("/api/auth/verify", { method: "POST", body: { channel: "email", target: ownerEmail, code: rc.json.devCode, intent: "register", password: "Test1234!ok", agreed: true }, auth: false });
...
await memberApi("/api/auth/verify", { method: "POST", body: { channel: "email", target: memberEmail, code: rc2.json.devCode, intent: "register", password: "Test1234!ok", agreed: true }, auth: false });
...
if (!mu.json?.saved?.length) throw new Error(`media upload failed: ${JSON.stringify(mu.json)}`);
check("media upload saved 1", mu.json?.saved?.length, 1, mu.json);
check("media upload no errors", (mu.json?.errors || []).length, 0, mu.json);
