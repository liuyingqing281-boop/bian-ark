...
// ---------- 6. 远端验证 + 报告 ----------
log("=== [6/6] 线上验证 ===");
const verify = run(`${SSH_BASE.join(" ")} "cd ${REMOTE_DIR} && node -e \\"console.log(JSON.stringify({
  commit: require('child_process').execSync('git rev-parse HEAD').toString().trim().slice(0,7),
  buildId: require('fs').readFileSync('.next/BUILD_ID','utf8').trim(),
  health: JSON.parse(require('child_process').execSync('curl -s http://localhost:3002/api/health')).status,
  starsea: require('child_process').execSync('curl -s -o /dev/null -w %{http_code} http://localhost:3002/api/garden/starsea?bbox=0,0,1,1').toString(),
  garden: require('child_process').execSync('curl -s -o /dev/null -w %{http_code} http://localhost:3002/zh/garden').toString(),
  protoBlocked: require('child_process').execSync('curl -s -o /dev/null -w %{http_code} http://localhost:3002/concept').toString() === '404',
  migrations: require('child_process').execSync('node tools/db-migrate.mjs status 2>/dev/null | grep -c applied || echo 0').toString().trim()
}))\\""`).trim());
let v = {};
try { v = JSON.parse(verify); } catch { log("  （验证输出解析失败，原文：\n" + verify + "）"); }
assert(v.health === "healthy", `health 异常：${v.health}`);
