/* 精剪逐帧渲染：/showreel?cut=N → N*30 帧 PNG（帧驱动，确定性输出）
 * 用法：SHOWREEL_URL=... CUT=12.5 node record-10s.cjs */
const { chromium } = require("@playwright/test");
const fs = require("fs");
const path = require("path");

const CUT = Number(process.env.CUT || 10);
const OUT_DIR = path.join(__dirname, "..", "..", "showreel", `frames-${CUT}s`);
const TOTAL_OUT = Math.round(CUT * 30);
...
(async () => {
  ...
  await page.goto(`${process.env.SHOWREEL_URL || "http://localhost:7102/showreel"}?cut=${CUT}`, { waitUntil: "networkidle" });
