...
- [x] 部署优化 P0/P1/P2/P3-1~3 全线落地（2026-09-07 完成并上线，master `84cfd4a`，见 docs/15 现状报告 + docs/16 优化方案与实施实录）：**P0-2 生产屏蔽原型/演示路由**（src/proxy.ts 门控 `ENABLE_PROTO_ROUTES`，生产缺省关闭→404；公网 /concept /proto /prototype /proto-zcode /showreel 实测 404，probe --check-proto-blocked 8/8）→ **P1 发布可靠性**（`tools/release.mjs` 一条龙：隔离构建 .next-release→RELEASE_COMMIT 产物-代码一致性→scp→apply-release→线上验证→报告落 deploy-reports/；`deploy/rollback.sh` 一键回滚；备份口径统一 data/backups；`npm run release` 全自动跑通两次 a0e41af/84cfd4a 验收）→ **P2 运维自动化**（`tools/probe.mjs` 外部拨测 + `.github/workflows/probe.yml` GitHub Runner 每 15 分钟外网探测、失败开 issue/恢复自关；`deploy/logrotate-bian.conf`(copytruncate) + `deploy/setup-ops.sh` 幂等安装：/etc/logrotate.d/bian + crontab 每日 04:00 db:backup & 04:05 清 7 天备份(# bian-ops) + pm2-root/pm2 save/certbot.timer 确认；**重启演练实测 22.5s 恢复 200**，nginx/pm2/cron/logrotate/P0-2 门控全自愈）→ **P3-1 首页收口**（用户拍板：/zh 保留概念首页+强化产品入口——主 CTA「进入星海」直进 /zh/garden、产品角注回声行、page.tsx 新增中英 SEO metadata）→ **P3-2 admin 鉴权统一中间件层**（规则单一源 `src/lib/admin-guard.ts`：canAccessAdmin+只读会话解析 resolveSessionEmail+SESSION_COOKIE；proxy 前置守卫 /api/admin* 未授权 403、/[lang]/admin* 未授权重定向登录；requireAdmin 复用同规则作纵深；db 路径抽 db-core.ts 共用；e2e 新增「admin 页匿名重定向」5/5 绿；线上实测 403/307）→ **P3-3 /uploads 流式化**（createReadStream+Readable.toWeb 流式回包、ETag/304 条件请求、Content-Length，不再 readFileSync 全量入内存；OSS Adapter 为后续独立项）。**实录修复**：release.mjs ① run() 遇 execSync stdio=inherit 返 null 崩溃 ② Windows 远端命令经 cmd 二次转义致 apply 快照 `cd /var/www/bian` 失败——远端两步改 argv 直传（a0e41af/84cfd4a）。**验证链**：tsc 零错、生产构建过（Proxy 捆绑 better-sqlite3 守卫 OK）、permissions e2e 5/5、生产模式本地实测（admin 403/307、uploads 304/200、/zh CTA）、发布报告瞬时 protoBlocked=no 系 PM2 新旧进程交接误报（复测 yes）、线上最终 probe 8/8 + commit 84cfd4a + 迁移 24。**待做**：P0-1 主账号 AK（LTAI…Qrmz）阿里云控制台禁用（需登录，步骤见 docs/16 表）、UptimeRobot 网页注册 5 分钟监控（docs/16 §P2-1 配置步骤）、probe.yml 首次调度观察、P3-4 文档归一（07 手册唯一真相源/删 deploy.sh/release-checklist 打通）
