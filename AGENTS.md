...
- [x] 星海正式前端实施落地（2026-09-02 完成，已合并 master `27ee718`）：按 `docs/superpowers/plans/2026-08-28-starsea-frontend-implementation.md` 9 任务 + 最终全分支评审 + 收尾修复波全数完成（16 提交 `57b92b5..27ee718`，每任务经独立实现/评审/修复复核闭环）。**交付**：迁移 025 固化馆级契约（建馆事务原子化、旧入园 API 委托馆级写路径、starsea API bbox/游标/limit=500 边界）；沉浸壳 AppFrame（/garden 与 /hall 去全局导航/聊天条/footer）；纯状态机 `src/lib/garden-sea(-state).ts`（URL 白名单 view/q/zone/hall/memorial/panel、镜头 sessionStorage、确定性星阵 starOffsets 禁随机）；正式 2.5D 星海 `src/components/starsea/` 六组件（44px 热区、24h 明灭语义、搜索防抖降亮不重排、三态抽屉、供奉 401/500 错误分支永不误报成功）；园→馆→人路由（/hall/[hallId] canonical + 旧 memorial URL 308 + ?p= 人物聚焦 + 进馆/返回镜头快照恢复 + PATCH 可见性同步 halls）；择位模式（显式拖拽、409 建议位吸附、MePanels 统一入口 ?placing=）；3D 增强层（three 按需 import 零基线成本、DOM overlay 承担语义交互、WebGL 丢失/reduced-motion/低性能/择位四路降级 2.5D）；性能规模（600+ 馆夹具 bbox 邻接/游标全覆盖证明、LOD、完整/简化/静态三档动效+用户可恢复、热区反缩放 ≥44px）；回归与文档（视觉基线 docs/shots/ 4 张、08 §3.13/09 M13/web/03/07 运维手册/release-checklist 同步、check-garden.mjs 重写 8 检查）。**收尾修复**：GET /api/halls/[id] 传输层脱敏（非馆主 members[].name/hall.name 下线，馆主原文）。**验证**：build/db:verify(24 迁移)/smoke p1·p2·p4·auth/Playwright 星海 110 过双工程/正式冒烟 `node --experimental-strip-types tools/test-starsea-formal.mjs` 30:0/tsc 零错全绿。**实现差异与备注**：garden.spec.ts 旧墓碑旅程并入 starsea.spec.ts 星群重写；e2e helpers 适配 auth intent/password 新契约（auth.spec 修复转绿）；Playwright webServer 注入 PORT=3002；旧 GardenScene/GardenViewSwitch/Garden3D 已删；多人馆全名搜索受脱敏限制（08 已记，待产品决策）；并行 e2e 建议 --workers=1（dev server Fast Refresh 类预存闪败）。待做：canViewHallFor 双实现去重、群组可见性分支 e2e、对外发布前重生成视觉基线
- [x] 部署优化 P0/P1/P2/P3-1~3 全线落地（2026-09-07 完成并上线，master `84cfd4a`，见 docs/15 现状报告 + docs/16 优化方案与实施实录）：**P0-2 生产屏蔽原型/演示路由**（src/proxy.ts 门控 `ENABLE_PROTO_ROUTES`，生产缺省关闭→404；公网 /concept /proto /prototype /proto-zcode /showreel 实测 404，probe --check-proto-blocked 8/8）→ **P1 发布可靠性**（`tools/release.mjs` 一条龙：隔离构建 .next-release→RELEASE_COMMIT 产物-代码一致性→scp→apply-release→线上验证→报告落 deploy-reports/；`deploy/rollback.sh` 一键回滚；备份口径统一 data/backups；`npm run release` 全自动跑通两次 a0e41af/84cfd4a 验收）→ **P2 运维自动化**（`tools/probe.mjs` 外部拨测 + `.github/workflows/probe.yml` GitHub Runner 每 15 分钟外网探测、失败开 issue/恢复自关；`deploy/logrotate-bian.conf`(copytruncate) + `deploy/setup-ops.sh` 幂等安装：/etc/logrotate.d/bian + crontab 每日 04:00 db:backup & 04:05 清 7 天备份(# bian-ops) + pm2-root/pm2 save/certbot.timer 确认；**重启演练实测 22.5s 恢复 200**，nginx/pm2/cron/logrotate/P0-2 门控全自愈）→ **P3-1 首页收口**（用户拍板：/zh 保留概念首页+强化产品入口——主 CTA「进入星海」直进 /zh/garden、产品角注回声行、page.tsx 新增中英 SEO metadata）→ **P3-2 admin 鉴权统一中间件层**（规则单一源 `src/lib/admin-guard.ts`：canAccessAdmin+只读会话解析 resolveSessionEmail+SESSION_COOKIE；proxy 前置守卫 /api/admin* 未授权 403、/[lang]/admin* 未授权重定向登录；requireAdmin 复用同规则作纵深；db 路径抽 db-core.ts 共用；e2e 新增「admin 页匿名重定向」5/5 绿；线上实测 403/307）→ **P3-3 /uploads 流式化**（createReadStream+Readable.toWeb 流式回包、ETag/304 条件请求、Content-Length，不再 readFileSync 全量入内存；OSS Adapter 为后续独立项）。**实录修复**：release.mjs ① run() 遇 execSync stdio=inherit 返 null 崩溃 ② Windows 远端命令经 cmd 二次转义致 apply 快照 `cd /var/www/bian` 失败——远端两步改 argv 直传（a0e41af/84cfd4a）。**验证链**：tsc 零错、生产构建过（Proxy 捆绑 better-sqlite3 守卫 OK）、permissions e2e 5/5、生产模式本地实测（admin 403/307、uploads 304/200、/zh CTA）、发布报告瞬时 protoBlocked=no 系 PM2 新旧进程交接误报（复测 yes）、线上最终 probe 8/8 + commit 84cfd4a + 迁移 24。**待做**：P0-1 主账号 AK（LTAI…Qrmz）阿里云控制台禁用（需登录，步骤见 docs/16 表）、UptimeRobot 网页注册 5 分钟监控（docs/16 §P2-1 配置步骤）、probe.yml 首次调度观察。**P3-4 文档归一已完成**（2026-09-07：07 手册升级为唯一真相源且主路径改 release.mjs 一条龙、deployment.md 标历史参考、docs/上线相关信息.md 运维应急同步新口径、`deploy/deploy.sh` 已 git rm、release-checklist 与 deploy-reports 报告打通）
- [x] 私有馆入园·馆主专属星海（2026-09-08 拍板并上线，master `d0261fb`）：产品规则变更——「只有公开馆才能入园」→ 私有馆也可择位入园，但星点**仅馆主本人登录可见**（星海按会话叠加：public 全访客、private 仅 owner_user_id=会话用户；匿名/他人路径 SQL 语义与旧版逐字节一致，无泄露面）。**代码**：`PATCH /api/halls/[id]/garden-pos` 与旧 `POST /api/memorials/[id]/garden` 移除 public 门禁（reason visibility_required 仅旧契约保留）；`GET /api/garden/starsea` 会话解析只读一次（热路径不做滑动续期）+ 可见性/灯数/烛光子查询 OR owner 动态装配。**验证**：tsc 零错、formal 冒烟 30/0（新增「owner 可入园、anon 不可见、owner 可见」断言；raw exit 0，PERF 探测超时为既有 skip 项）、release.mjs 一条龙发布 d0261fb（build/一致性/迁移/冒烟 4 PASS）、线上双视角实测：anon=1(仅小曼)、owner1=4(小鱼/3/4+小曼)、owner2=2(小鱼6号+小曼)、私有不泄漏=true。**生产数据动作**：名下 4 座私有测试馆已择位入园（小鱼 .20,.30 / 小鱼3号 .42,.68 / 小鱼4号 .70,.22 / 小鱼6号 .85,.62，间距>0.04 合规；ly 不动），星海对访客仍只见公开的小曼（蝉）。**文档**：08 §3.13 / 09 in_garden / web/02 §八 / web/03 §3.5 / garden-sea.ts 注释同步。**待做**：无（如需可见需在对应账号登录进 /zh/garden）
